...
import HashControlFactory from "../../../core/hash-control/HashControlFactory";
...
class Authenticate extends BaseUseCase {
  ...
  private requestBody: RequestModel.authentication;
  ...
  async execute() {
    try {
      ...
      if (email) {
        ...
        console.log(">>>>>>>>>>>>>", user)
        if (user) {
          let isPasswordMatched = await this.hashControl.compare(
            password,
            user.password
          );
  ...
  static create(request, response) {
    ...
    const useCase = new Authenticate(
      request,
      response,
      hashControl,
      authControl,
